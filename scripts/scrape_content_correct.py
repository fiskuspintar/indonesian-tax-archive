#!/usr/bin/env python3
"""
FULL CONTENT SCRAPER - CORRECT ORTAX IDs VERSION
This script fetches full document content using the REAL Ortax IDs.
"""
...
PROGRESS_FILE = DATA_DIR / 'content_scrape_progress_correct.json'
LOG_FILE = DATA_DIR / 'content_scrape_correct.log'

# Load regulations with CORRECT Ortax IDs
print("[INIT] Loading regulations with CORRECT Ortax IDs...")
regulations = []
...
print(f"[INIT] Loaded {TOTAL} regulations with correct IDs")
...
async def fetch_regulation_content(page, regulation_id, retries=3):
    """Fetch content for a single regulation with retries"""
            ...
            if not content:
                try:
                    ...
                    if body:
                        ...
                        content_lines = [l for l in lines if len(l) > 100]
                        if content_lines:
                            content = '\n\n'.join(content_lines[:50])
                except:
                    pass
            
            return content[:50000] if content else None
...
async def scrape_all_content():
    """Fetch content for ALL regulations using CORRECT IDs"""
    log("=" * 70)
    log("FULL CONTENT SCRAPER - CORRECT ORTAX IDs - STARTING")
    ...
    async with async_playwright() as p:
        ...
        log("[INIT] Browser started, beginning scrape with CORRECT IDs...")
        
        # Process all regulations
        for i in range(start_index, TOTAL):
            reg = regulations[i]
            
            # Get the REAL Ortax ID from source_url or api_id
            ...
            content = await fetch_regulation_content(page, ortax_id)
            
            if content:
                reg['full_content'] = content
                log(f"  ✓ Content fetched: {len(content)} characters")
            else:
    ...
    log(f"Without content: {len(enriched) - with_content}")
